...
const psalms = load('library/psalms_full.json') as Record<string, JsonRecord>
...
export function assembleDailyPrayer({ day, date, season }: { day: number; date: Date | string; season?: string }) {
  ...
  return {
    ...
    vespers: { opening, psalm: psalm(psalmOfTheDay.number === 134 ? 'psalm_4' : office.hours.vespers.psalmOrCanticle), canticle: canticles[office.hours.vespers.canticleId as keyof typeof canticles], shortPrayer: seasonData.shortPrayer, blessing: blessings.blessing_vespers },
    compline: { opening, psalm: psalm(office.hours.compline.fixedPsalmIds[1]), canticle: canticles[office.hours.compline.canticleId as keyof typeof canticles], nightPrayer: office.commonTexts.nightPrayer, blessing: blessings.blessing_compline },
  }
}
...
export function prayerToBlocks(prayer: DailyPrayer, hour: 'lauds' | 'vespers' | 'compline'): PrayerBlock[] {
  ...
  if (hour === 'lauds') {
    ...
    blocks.push({ heading: `Psalm ${prayer.vespers.psalm.number}`, text: withoutVerseNumbers(prayer.vespers.psalm.text) }, { heading: 'Magnificat · Canticle · Luke 1:46-55', text: withoutVerseNumbers(prayer.vespers.canticle.text) }, { heading: 'Short Prayer', text: prayer.vespers.shortPrayer.text }, { heading: 'Blessing', text: prayer.vespers.blessing.text })
  } else {
